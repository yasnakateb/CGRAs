...
class CellProcessing 
    ...
    val FU_INST = Module (new FU(DATA_WIDTH, 4))
    ...
    val SEB_OUT = Module (new D_SEB(DATA_WIDTH))
    SEB_OUT.io.din := FU_dout
    SEB_OUT.io.din_v := FU_dout_v
    FU_dout_r := SEB_OUT.io.din_r
    io.dout := SEB_OUT.io.dout   
    io.dout_v := SEB_OUT.io.dout_v 
    SEB_OUT.io.dout_r := forked_dout_r
